...
public class AddTripActivity extends AppCompatActivity {
    ...
    private void saveTripData() {
        ...
        calculateGroupMBTI(selectedFriendsIds, teamMBTI -> {
            ...
            prompt.append(teamMBTI).append("에 I 있으면 실내 추천\n O있으면 자연경관, 야외, 바다와 같은 야외 추천");
            prompt.append(teamMBTI).append("에 M있으면 박물관, 체험관 추천 필수");
            prompt.append(teamMBTI).append("에 F 있으면 날마다 카페 2곳 추가\n M 있으면 카페 절대 추가 금지ㅡ.");
            prompt.append("음식점은 날마다 2곳 추가.\n");
            prompt.append("음식점/카페 제외 관광지와 쇼핑몰, 자연경관 등을 추천하여 일정에 추가 필수\n");
    ...
    private String getGroupMBTIStyle(String groupMBTI) {
        ...
        String scheduleDesc = (scheduleCount == 'T') ? "일정은 13개" : (scheduleCount == 'L') ? "일정은 딱 4개" : "";
